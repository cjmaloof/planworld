...
class Archive {
   ...
   */
  static function _get ($col, &$uid, $ts=null, $idx=0) {
    ...
    if ($result) {
      // If idx > 0, throw away earlier rows
      for ($i=0; $i < $idx && $result->fetch(); $i++) {
      }
      $row = $result->fetch();
      if (!$row) {
   ...
   */
  static function _set ($col, $val, $uid, $ts, $low_priority=false) {
    $dbh = DBUtils::_connect();
   ...
   */
  static function getEntryByIndex ($uid, $idx) {
   ...
   */
  static function saveEntry ($uid, $ts, $content, $name='', $public=false) {
    $dbh = DBUtils::_connect();
    ...
    $result = $dbh->query($query);
    if ($result) {
      if ($result->rowCount() < 1) return ARCHIVE_ERROR;
   ...
   */
  static function listEntries ($uid, $public=true, $start=null, $end=null) {
    $dbh = DBUtils::_connect();
   ...
   */
  static function setPublic ($uid, $ts) {
    if (is_array($ts) && sizeof($ts) > 0) {
      $dbh = DBUtils::_connect();
   ...
   */
  static function setPrivate ($uid, $ts) {
    if (is_array($ts) && sizeof($ts) > 0) {
      $dbh = DBUtils::_connect();
   ...
   */
  static function isPublic ($uid, $ts) {
   ...
   */
  static function isPrivate ($uid, $ts) {
   ...
   */
  static function getName ($uid, $ts) {
   ...
   */
  static function setName ($uid, $ts, $name) {
  ...
  static function getSize ($uid) {
    ...
    if ($result) {
      $row = $result->fetch();
      if ($row == false) return ARCHIVE_ERROR;
      return $row['size'];
    } else {
      return ARCHIVE_ERROR;
    }
   ...
   */
  static function getViews ($uid, $ts) {
   ...
   */
  static function addView ($uid, $ts) {
   ...
   */
  static function hasPublicEntries ($uid) {
    $dbh = DBUtils::_connect();
    ...
    if ($result) {
      $row = $result->fetch();
      if ($row == false) return ARCHIVE_EMPTY;
      if ($row['size'] > 0)
        return true;
      else
        return false;
   ...
   */
  static function sortList (&$list, $sortby, $reverse=false) {
    switch ($sortby) {
    case 'ts':
